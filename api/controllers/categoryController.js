...
const getAllCategories = async (req, res) => {
  let categoriesList = await fetch(`http://api.sr.se/api/v2/programcategories/?${jsonFormat}&${paginationFalse}`);
...
const getAllProgramsInCategory = async (req,res) => {
  let programList = await fetch(`http://api.sr.se/api/v2/programs/index?${jsonFormat}&${paginationFalse}&programcategoryid=${req.params.categoryId}`);
